...
const isSentinelPx = (v) => v == null || isNaN(v) || v >= 9000 || v <= -9000;
const BG = [8, 8, 12, 220]; // fondo para pixels fuera de la máscara

/* misma lógica que buildCIR pero oscurece los pixels excluidos por la máscara */
export function buildCIRMasked(nir, red, green, ndviMasked) {
  let nirMn = Infinity, nirMx = -Infinity;
  let redMn = Infinity, redMx = -Infinity;
  let grnMn = Infinity, grnMx = -Infinity;
  for (let y = 0; y < RAW_H; y++)
    for (let x = 0; x < RAW_W; x++) {
      if (isSentinelPx(ndviMasked[y]?.[x])) continue;
      const n = nir[y]?.[x], r = red[y]?.[x], g = green[y]?.[x];
      if (n != null && isFinite(n)) { if (n < nirMn) nirMn = n; if (n > nirMx) nirMx = n; }
      if (r != null && isFinite(r)) { if (r < redMn) redMn = r; if (r > redMx) redMx = r; }
      if (g != null && isFinite(g)) { if (g < grnMn) grnMn = g; if (g > grnMx) grnMx = g; }
  ...
  for (let y = 0; y < RAW_H; y++)
    for (let x = 0; x < RAW_W; x++) {
      const i = (y * RAW_W + x) * 4;
      if (isSentinelPx(ndviMasked[y]?.[x])) {
        id[i]=BG[0]; id[i+1]=BG[1]; id[i+2]=BG[2]; id[i+3]=BG[3];
      } else {
        id[i]   = norm(nir[y]?.[x]   ?? nirMn, nirMn, nirMx);
        id[i+1] = norm(red[y]?.[x]   ?? redMn, redMn, redMx);
        id[i+2] = norm(green[y]?.[x] ?? grnMn, grnMn, grnMx);
...
/* misma lógica que buildNDVI pero oscurece los pixels excluidos por la máscara */
export function buildNDVIMasked(ndvi, ndviMasked) {
  const id = new Uint8ClampedArray(RAW_W * RAW_H * 4);
  for (let y = 0; y < RAW_H; y++)
    for (let x = 0; x < RAW_W; x++) {
      const i = (y * RAW_W + x) * 4;
